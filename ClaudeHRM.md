...
### BL-001 설계 방향 (2026-06-08 Claude.ai 합의) — Phase 2 PRE 출발점

**채택 방향**: B안(OKR 고유 철학 구현) + MBO 엔진 구조 재사용.

**OKR 철학 2원칙 (반드시 시스템·정책에 반영):**
1. 보상 분리 — OKR 점수로 등급·보상을 **자동 산출하지 않음** (정성 참고만). 핵심: sandbagging 방지 = 도전의 심리적 안전.
2. 상대평가 금지 — OKR로 서열화·동료 비교 안 함. 절대평가 시 매니저 맥락 참고는 가능하나 기계적 산식 없음.
→ 두 원칙을 **공개적으로 선언**해야 철학 성립.

**점수 노출**: 개인 OKR 내용(Objective/KR/진행률)은 전사 공개 / 점수 통계는 경영자만(경영의사결정 참고).

**엔진 구현 방식 (단순 카피 아님):**
- MBO 2계층 구조 재사용: O→카테고리, KR→목표+가중치, 점수 산출 로직 재사용.
- **등급 자동 산출·보상 연동 제거** (MBO 엔진에 딸려오는 연동을 의도적으로 끊음).
- 공개 모델 교체: 상사-부하 라인 → 전사 공개.
- 정통 OKR 점수 철학 반영: stretch(70% 정상), 등급 매핑·정규화 OKR용 별도.

**개발 순서 (대시보드 마지막):**
1. OKR 평가 엔진 (MBO 구조 재사용 + 연동 제거)
2. 보상 비연동 + 상대평가 금지 공개 선언 (정책·UI 문구)
3. 점수 경영자 통계 뷰
4. 전사 공개 OKR 대시보드 (대문에 OKR 선언문 배치) ← 마지막

**OKR 선언문 (대시보드 대문용 초안 — 문구 추후 확정):**
> 하나. OKR은 도전을 위한 것입니다. 70% 달성은 실패가 아니라 정상입니다.
> 둘. OKR은 보상을 결정하지 않습니다. 점수로 등급·보상을 산정하지 않습니다.
> 셋. 우리는 서열을 매기지 않습니다.
> 넷. OKR은 모두에게 열려 있습니다. 누구나 어떤 조직의 OKR이든 볼 수 있습니다.
> 다섯. 점수는 방향을 보기 위한 신호입니다. 평가가 아닙니다.
> — 도전은 안전합니다. 실패는 배움입니다. 우리는 함께 더 높은 곳을 봅니다.

**마케팅 연계(BL-003)**: B안 채택 시 "진짜 OKR 지원"으로 표현 가능(현재는 MBO만). A안(MBO 매핑)이면 "OKR 지원" 표현 애매 — 표현 정확화 필수.

**시점**: 운영 게이트(A1~A3) 이후 Phase 2. 본 블록이 전용 PRE 출발점.
